...
export function ProductsManagementPage() {
  ...
  const saveProduct = async () => {
    ...
    if (!form.name || !form.description || Number.isNaN(parsedPrice)) return
    if (mode === "add" && !form.imageFile) return
    ...
    try {
      ...
      if (form.imageFile) requestBody.append("image", form.imageFile)
      if (mode === "edit") requestBody.append("existingImageUrl", form.existingImageUrl)

      const url =
        mode === "add" ? "/api/products" : `/api/products/${editingId ?? ""}`
      const method = mode === "add" ? "POST" : "PUT"
      const response = await fetch(url, { method, body: requestBody })
      const payload = await response.json()
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setErrorMessage(payload.message ?? "Failed to delete product")
        return
      }
